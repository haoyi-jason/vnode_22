...
static THD_WORKING_AREA(waMBTask,1024);
static THD_FUNCTION(thMBTask ,p)
{
  ...
  else address = 0xf - address +1;
  
  
  if(MB_RTU_EN){
    if(!opt){
      if(eMBSSerialInit(&xMBSHdl,MB_RTU,0x1,0,9600,MB_PAR_NONE) != MB_ENOERR){
      ...
      if(eMBSSerialInit(&xMBSHdl,MB_RTU,opt->sla,0,opt->baudrate*100,opt->parity) != MB_ENOERR){
